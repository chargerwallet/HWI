...
VENDORS = ("chargerwallet.com", )


class ChargerwalletFeatures(Features):
...
DEFAULT_MAPPING.register(ChargerwalletFeatures)

USB_IDS = {(0x1209, 0x4F4A), (0x1209, 0x4F4B), }

CHARGERWALLET_LEGACY = TrezorModel(
    name="1",
...
CHARGERWALLET_TOUCH = TrezorModel(
    name="T",
...
CHARGERWALLETS = (CHARGERWALLET_LEGACY, CHARGERWALLET_TOUCH)


def model_by_name(name: str) -> Optional[TrezorModel]:
    for model in CHARGERWALLETS:
        if model.name == name:
            return model
    return None


# ===============overwrite methods for chargerwallet device begin============
...
def _refresh_features(self: object, features: Features) -> None:
    ...
    if not self.model:
        ...
        if self.model is None:
            raise RuntimeError("Unsupported ChargerWallet model")

    if features.vendor not in self.model.vendors:
        raise RuntimeError("Unsupported device")
    self.features = features
    self.version = (*map(int, self.features.chargerwallet_version.split(".")), )
...
def button_request(self: object, code: Optional[int]) -> None:
    if not self.prompt_shown:
        print("Please confirm action on your ChargerWallet device", file=sys.stderr)
    if not self.always_prompt:
        self.prompt_shown = True


# ===============overwrite methods for chargerwallet device end============

CHARGERWALLET_EMULATOR_PATH = "127.0.0.1:54935"
class ChargerwalletClient(TrezorClient):
    def __init__(
        self,
        path: str,
        ...
        super().__init__(path, password, expert, chain, webusb_ids=USB_IDS, sim_path=CHARGERWALLET_EMULATOR_PATH)
...
def enumerate(
    ...
    devs.extend(udp.UdpTransport.enumerate(path=CHARGERWALLET_EMULATOR_PATH))
    for dev in devs:
        d_data: Dict[str, Any] = {}

        d_data["type"] = "chargerwallet"
        d_data["path"] = dev.get_path()
        client = None
        with handle_errors(common_err_msgs["enumerate"], d_data):
            client = ChargerwalletClient(d_data["path"], password)
            try:
                client._prepare_device()
            except TypeError:
                continue
            if not client.client.features.chargerwallet_version or client.client.features.vendor not in VENDORS:
                continue

            d_data["label"] = client.client.features.label
            d_data["model"] = "chargerwallet_" + client.client.features.model.lower()
            ...
            if d_data["needs_pin_sent"]:
                raise DeviceNotReadyError(
                    "ChargerWallet is locked. Unlock by using 'promptpin' and then 'sendpin'."
                )
