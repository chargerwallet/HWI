...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        ;;
        --chargerwallet-1)
        build_chargerwallet_1=1
        shift
        ;;
        --chargerwallet-t)
        build_chargerwallet_t=1
        shift
        ;;
        --coldcard)
        build_coldcard=1
...
if [[ -n ${build_chargerwallet_1} || -n ${build_chargerwallet_t} ]]; then
    # Clone chargerwallet-firmware if it doesn't exist, or update it if it does
    if [ ! -d "chargerwallet-firmware" ]; then
    ...
    if [[ -n ${build_chargerwallet_1} ]]; then
        # Build trezor one emulator. This is pretty fast, so rebuilding every time is ok
        # But there should be some caching that makes this faster
    ...
    if [[ -n ${build_chargerwallet_t} ]]; then
        rustup toolchain uninstall nightly
        rustup toolchain install nightly
        rustup default nightly
        # Build trezor t emulator. This is pretty fast, so rebuilding every time is ok
        ...
        poetry run make build_unix
        # Delete any emulator.img file
        find . -name "chargerwallet.flash" -exec rm {} \;
        cd ..
    fi
    cd ..
fi
