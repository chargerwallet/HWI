...
- `test_chargerwallet.py` tests the command line interface and the Chargerwallet implementation.
It uses the [Chargerwallet Legacy firmware emulator](https://github.com/chargerwallet/firmware/blob/bixin_dev/docs/legacy/index.md#local-development-build) and [ChargerWallet Touch emulator](https://github.com/chargerwallet/firmware/blob/touch/docs/core/emulator/index.md).
It also tests usage with `bitcoind`.
...
`setup_environment.sh` will build the Trezor emulator, the Chargerwallet emulator, the Coldcard simulator, the Keepkey emulator, the Digital Bitbox simulator, the Jade emulator, and `bitcoind`.
if run in the `test/` directory, these will be built in `work/test/trezor-firmware`, `work/test/chargerwallet-firmware`, `work/test/firmware`, `work/test/keepkey-firmware`, `work/test/mcu`, and `work/test/bitcoin` respectively.
In order to build each simulator/emulator, you will need to use command line arguments.
These are `--trezor-1`, `--trezor-t`, `--chargerwallet-1`, `--chargerwallet-t`, `--coldcard`, `--keepkey`, `--bitbox01`, `--jade`, and `--bitcoind`.
...
`test_trezor.py`, `test_chargerwallet.py`, `test_coldcard.py`, `test_keepkey.py`, `test_jade.py`, and `test/test_digitalbitbox.py` can be disabled.
...
## Chargerwallet emulator

### Dependencies

In order to build the Chargerwallet emulator, the [Nix](https://nixos.org) will need to be installed:
...
```
$ git clone --recursive https://github.com/chargerwallet/firmware.git chargerwallet-firmware
```

For the Chargerwallet Legacy firmware emulator:
```
...
```
For the Chargerwallet Touch emulator:
```
$ git checkout touch
$ cd chargerwallet-firmware
$ nix-shell
