...
from hwilib.devices.chargerwallet import _refresh_features
...
class OnkeyEmulator(DeviceEmulator):
    def __init__(self, path, model):
        assert model in CHARGERWALLET_MODELS
        self.emulator_path = path
        self.emulator_proc = None
        self.model = model
        self.emulator_log = None
        try:
            os.unlink('chargerwallet-{}-emulator.stdout'.format(self.model))
        except FileNotFoundError:
            pass
        self.type = f"chargerwallet_{model}"
    ...
    def start(self):
        super().start()
        self.emulator_log = open('chargerwallet-{}-emulator.stdout'.format(self.model), 'a')
        # Start the Chargerwallet emulator
    ...
    def stop(self):
        ...
        if self.model == 't':
            emulator_img = "/var/tmp/chargerwallet.flash"
        else:  # self.model == '1'
...
class ChargerwalletTestCase(unittest.TestCase):
    def __init__(self, emulator, interface='library', methodName='runTest'):
        super(ChargerwalletTestCase, self).__init__(methodName)
    ...
    def __str__(self):
        return 'chargerwallet_{}: {}'.format(self.emulator.model, super().__str__())

    def __repr__(self):
        return 'chargerwallet_{}: {}'.format(self.emulator.model, super().__repr__())
...
# ChargerWallet specific getxpub test because this requires device specific thing to set xprvs
class TestChargerwalletGetxpub(ChargerwalletTestCase):
    def test_getxpub(self):
        ...
        for vec in vectors:
            with self.subTest(vector=vec):
                ...
                # Test getmasterxpub
                gmxp_res = self.do_command(['-t', 'chargerwallet', '-d', "udp:127.0.0.1:54935", 'getmasterxpub', "--addr-type", "legacy"])
                self.assertEqual(gmxp_res['xpub'], vec['master_xpub'])

                # Test the path derivs
                for path_vec in vec['vectors']:
                    gxp_res = self.do_command(['-t', 'chargerwallet', '-d', "udp:127.0.0.1:54935", 'getxpub', path_vec['path']])
                    self.assertEqual(gxp_res['xpub'], path_vec['xpub'])

    def test_expert_getxpub(self):
        result = self.do_command(['-t', 'chargerwallet', '-d', "udp:127.0.0.1:54935", '--expert', 'getxpub', 'm/44h/0h/0h/3'])
...
class TestChargerwalletLabel(ChargerwalletTestCase):
    def setUp(self):
        self.client = self.emulator.start()
        self.dev_args = ['-t', 'chargerwallet', '-d', "udp:127.0.0.1:54935"]

    def test_label(self):
        result = self.do_command(self.dev_args + ['enumerate'])
        for dev in result:
            if dev['type'] == 'chargerwallet' and dev['path'] == "udp:127.0.0.1:54935":
                self.assertEqual(dev['label'], 'test')
                break
        else:
            self.fail("Did not enumerate device")

def chargerwallet_test_suite(emulator, bitcoind, interface, model):
    assert model in CHARGERWALLET_MODELS
    ...
    suite = unittest.TestSuite()
    suite.addTest(DeviceTestCase.parameterize(TestDeviceConnect, bitcoind, emulator=dev_emulator, interface=interface, detect_type="chargerwallet"))
    suite.addTest(DeviceTestCase.parameterize(TestDeviceConnect, bitcoind, emulator=dev_emulator, interface=interface, detect_type=f"chargerwallet_{model}_simulator"))
    ...
    suite.addTest(ChargerwalletTestCase.parameterize(TestChargerwalletLabel, emulator=dev_emulator, interface=interface))
    suite.addTest(ChargerwalletTestCase.parameterize(TestChargerwalletGetxpub, emulator=dev_emulator, interface=interface))
    result = unittest.TextTestRunner(stream=sys.stdout, verbosity=2).run(suite)
    sys.stderr = sys.__stderr__
    return result.wasSuccessful()

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Test Chargerwallet implementation')
    parser.add_argument('emulator', help='Path to the Chargerwallet emulator')
    parser.add_argument('bitcoind', help='Path to bitcoind binary')
    parser.add_argument('--interface', help='Which interface to send commands over', choices=['library', 'cli', 'bindist'], default='library')
    group = parser.add_argument_group()
    group.add_argument('--model_1', help='The emulator is for the Chargerwallet legacy', action='store_const', const='1', dest='model')
    group.add_argument('--model_t', help='The emulator is for the Chargerwallet Touch', action='store_const', const='t', dest='model')
    args = parser.parse_args()

    # Start bitcoind
    bitcoind = Bitcoind.create(args.bitcoind)

    sys.exit(not chargerwallet_test_suite(args.emulator, bitcoind, args.interface, args.model))
